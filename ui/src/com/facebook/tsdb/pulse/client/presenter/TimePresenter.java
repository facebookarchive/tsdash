 ...
 */
package com.facebook.tsdb.pulse.client.presenter;
...
import com.facebook.tsdb.pulse.client.model.TimeRange;
import com.facebook.tsdb.pulse.client.model.ApplicationState.TimeMode;
...
public class TimePresenter implements Presenter {
	...
	private HashMap<Object, Long> historyButton2Range = 
			new HashMap<Object, Long>(); 
	private TreeMap<Long, Object> historyRange2Button = 
	...
	private void bindWidget() {
		...
		ClickHandler historyOptionsHandler = new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				TimeRange timeRange = new TimeRange(historyButton2Range.get(
						event.getSource()));
				// sync the absolute time elements
				widget.timeFromValue().setValue(new Date(timeRange.from), 
						false);
				widget.timeToValue().setValue(new Date(timeRange.to),
						false);
		...
		widget.timeFrom().addValueChangeHandler(
			new ValueChangeHandler<Date>() {
				...
				public void onValueChange(ValueChangeEvent<Date> event) {
					...
					TimeRange range = new TimeRange(event.getValue().getTime(),
							to.getTime()); 
					eventBus.fireEvent(new TimeRangeChangeEvent(
						TimeMode.ABSOLUTE, range)
					);
					widget.selectedHistory(null);
				}
		});
		widget.timeTo().addValueChangeHandler(
				new ValueChangeHandler<Date>() {
					@Override
					public void onValueChange(ValueChangeEvent<Date> event) {
						...
						if (from.after(event.getValue())) {
							...
							widget.timeToValue().setValue(new Date(), 
									false);
							return;
						}
						TimeRange range = new TimeRange(from.getTime(),
	...
	private void selectHistoryRange(TimeRange timeRange) {
		...
		long range = 0;
		for (Iterator<Long> it = historyRange2Button.keySet().iterator(); 
			 it.hasNext(); ) {
			range = it.next(); 
	...
	private void initTimeWidget(ApplicationState appState) {
		...
		widget.timeToValue().setValue(new Date(appState.timeRange.to), 
				false);
		widget.timeFromValue().setValue(
				new Date(appState.timeRange.from), false);
